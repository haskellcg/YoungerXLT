...
namespace ConsoleDB
{
    class DBFunction
    {
        ...
        public bool UserExists(string userID)
        {
			...
			//string cmdText = "select * from younger_user_info where user_id=?";
        ...
        public int InsertRelationship(Relationship relat)
        {
            ...
            int insertInc=dbc.ExecuteNonQueryForInc(cmdText);
            dbc.Close();
            return insertInc;
        }
        //参看好友关系ID是否存在
        ...
        //删除好友,表示user1删除好友user2
        public void DeleteFriend(string user1,string user2)
        {
            DBConnection dbc=new DBConnection();
            dbc.GetConnection();
            System.Console.WriteLine(IsFriend(user1,user2));
            if (IsFriend(user1, user2))
            {
                string cmdText ="delete from younger_relat_info where relat_user1='" + user1 + "' && relat_user2='" + user2 + "' && relat_confirm=1";
                System.Console.WriteLine(cmdText);
                dbc.ExecuteNonQuery(cmdText);
                cmdText = "delete from younger_relat_info where relat_user1='" + user2 + "' && relat_user2='" + user1 + "' && relat_confirm=1";
                System.Console.WriteLine(cmdText);
                dbc.ExecuteNonQuery(cmdText);
        ...
        public List<GroupInfo> JoinedGroups(string userID)
        {
            ...
            List<GroupInfo> list = new List<GroupInfo>();
            dbc.GetConnection();
            string cmdText = "select entity_groupid from younger_entity_info where entity_memid='"+userID+"'&& entity_confirm=1";
            dbc.GetDataReader(cmdText);
            while (dbc.dataReader.Read())
            {
                GroupInfo groupTemp = new GroupInfo();
        ...
        public bool EntityExists(int groupID, string userID)
        {
            ...
            DBConnection dbc = new DBConnection();
            dbc.GetConnection();
            string cmdText = "select * from younger_entity_info where entity_groupid="+groupID+"&&entity_memid='"+userID+"'";
            dbc.GetDataReader(cmdText);
            if (dbc.dataReader.Read())
            {
                flag = true;
